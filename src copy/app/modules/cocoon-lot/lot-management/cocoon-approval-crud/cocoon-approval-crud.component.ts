...
@Component({
  selector: 'app-cocoon-approval-crud',
  templateUrl: './cocoon-approval-crud.html',
  styleUrls: ['./cocoon-approval-crud.component.scss'],
  providers: [
...
export class CocoonApprovalCRUDComponent {
   ...
   // this.getmandiList();
    ...
    this.route.params.subscribe((params: Params) => {
      if (params['id']) {
        ...
        this.getStagingEntitesByid(this.ticketId )
  ...
  getStagingEntitesByid(tktId){
    let searchQuery = "externalId=" + tktId;
    this.api.getTicketDetails(searchQuery).then(res=>{
     
      
   // this.api.getStagingEntitiesById(reqObj).then(res=>{
      this.lotDetails = res[0];
          console.log('this.lotDetails',this.lotDetails);
          var userSelectedGrade=this.lotDetails.data.userSelectedGrade;

          this.execLimit=userSelectedGrade.conditions[0].value[1];
          this.l1Limit=userSelectedGrade.conditions[1].value[1];
          this.l2Limit=userSelectedGrade.conditions[2].value[1];
          this.approvalReason=this.lotDetails.data.reason;
          this.approvedBy=this.lotDetails.data.ApprovedBy;
          
          this.lotCreateForm.patchValue(this.lotDetails?.stagingEntities?.data);
          this.selectedFarmer = this.lotDetails?.stagingEntities?.data?.farmer?.id;          
          this.onCocoonTypeChange();
          this.onGradeValueChanges();

          this.approvedPrice = this.lotDetails?.data?.approvedPrice ? this.lotDetails?.data?.approvedPrice: 0;
          if(this.lotDetails?.stagingEntities.status == 'APPROVED'){
            this.lotCreateForm.get('receivedWeightPricePerKg').patchValue(this.approvedPrice);
          }else{
            this.lotCreateForm.get('receivedWeightPricePerKg').patchValue(this.lotDetails?.stagingEntities?.data?.userSelectedGrade.event.params.recomendedRate);
          }
         


          this.lotCreateForm.get('recomendedRate').patchValue(this.lotDetails?.stagingEntities?.data?.userSelectedGrade.event.params.recomendedRate);

          this.lotCreateForm.get('backupCenterCocoonWeightLoss').patchValue(this.lotDetails?.stagingEntities?.data['centerCocoonWeightLoss']);
          this.lotCreateForm.get('center').patchValue(this.lotDetails?.stagingEntities?.data?.SelectedcenterID);
          this.addressForm.addressForm.get('pincode').patchValue(this.lotDetails?.stagingEntities?.data?.farmer?.address?.pincode)
          this.addressForm.addressForm.get('address').patchValue(this.lotDetails?.stagingEntities?.data?.farmer?.address?.address)
          this.addressForm.addressForm.get('village').patchValue(this.lotDetails?.stagingEntities?.data?.farmer?.address?.village)
          this.addressForm.addressForm.get('city').patchValue(this.lotDetails?.stagingEntities?.data?.farmer?.address?.city)
        
          // let representiveObj;
          // if (this.lotDetails?.stagingEntities?.data?.rmRepresentativeName) {
          //   representiveObj = {
          //     name: this.lotDetails?.stagingEntities?.data?.rmRepresentativeName,
          //     phone: this.lotDetails?.stagingEntities?.data?.rmRepresentativePhone
          //   }
          // } else {
          //   representiveObj = {
          //     name: '-',
          //     phone: this.lotDetails?.stagingEntities?.data?.createdBy
          //   }
          // }
          // this.lotCreateForm.get('representive').patchValue(representiveObj);
          this.lotCreateForm.get('chaakiCenterName').patchValue(this.lotDetails?.stagingEntities?.data?.farmer?.chaakiCenterName);
          this.lotCreateForm.get('chaakiCenterPhone').patchValue(this.lotDetails?.stagingEntities?.data?.farmer?.chaakiCenterPhone);
          if(this.lotDetails?.stagingEntities?.data['cocoonRendittaId']) {
            this.api.getRendittaGradingById(this.lotDetails?.stagingEntities?.data['cocoonRendittaId']).then(gradingData=> {
          ...
          this.lotCreateForm.get('grade').patchValue(this.lotDetails?.stagingEntities?.data['grade']);
          this.lotCreateForm.get('rmGrade').patchValue(this.lotDetails?.stagingEntities?.data['rmGrade']);
          this.lotCreateForm.get('farmer').patchValue(this.lotDetails?.stagingEntities?.data['farmer']['id']);
          let farmerDetails = this.enterFarmerDetails(this.lotDetails?.stagingEntities?.data['farmer']['id'], false, false);
          if (this.lotDetails?.stagingEntities?.data['farmer']['bankDetails'][0] != null) {
           this.bankForm.bankForm.get('bankName').patchValue(this.lotDetails?.stagingEntities?.data['farmer']['bankDetails'][0]['bankName']);
           this.bankForm.bankForm.get('accountNumber').patchValue(this.lotDetails?.stagingEntities?.data['farmer']['bankDetails'][0]['accountNumber']);
           this.bankForm.bankForm.get('beneficiaryName').patchValue(this.lotDetails?.stagingEntities?.data['farmer']['bankDetails'][0]['beneficiaryName']);
           this.bankForm.bankForm.get('ifscCode').patchValue(this.lotDetails?.stagingEntities?.data['farmer']['bankDetails'][0]['ifscCode']);
           this.bankForm.bankForm.get('branchName').setValue(this.lotDetails?.stagingEntities?.data['farmer']['bankDetails'][0]['branchName']);

          }
          if (this.lotDetails?.stagingEntities?.data.couponAmount > 0) {
            this.redeemCode(this.lotDetails?.stagingEntities?.data);
          }
          // this.getCouponCodeForAFarmer()
          // if (this.lotDetails?.stagingEntities?.data['couponAmount'] && this.lotDetails?.stagingEntities?.data['couponAmount'] > 0) {
          //   this.lotCreateForm.get('couponAmt').patchValue(this.lotDetails?.stagingEntities?.data['couponAmount']);
          //   this.couponMsg = this.lotDetails?.stagingEntities?.data['couponAmount'] + ' Added Suceessfully. '
  ...
  async getCoonLotDetail(lotId) {
    this.ngxLoader.stop();
    this.api.getStagingEntitiesById(lotId).then((response:any) => {
      if (response) {
        if(response['uom']=='PCS'){
          this.uom = 'PCS';
  ...
  enterFarmerDetails(farmerId, patchBankDetails, checkName) {
    ...
    this.api.getFarmerById(farmerId).then(res => {
      ...
      if(checkName) {
        this.selectedFarmerDetails = res;
        console.log(this.selectedFarmerDetails);
    ...
    if (this.lotId) {
      ...
      this.api.putCocoonLot(this.lotId, params).then(res => {
        ...
        if (res) {
          ...
          this.api.updateStagingEntitiesById(this.lotDetails?.stagingEntities.id, {"lotStatus" : "CREATED"}).then(data => {
              console.log("updated Staging entity success fully");
      ...
      this.api.postCocoonLot(params).then((res:any) => {
        ...
        this.api.updateStagingEntitiesById(this.lotDetails?.stagingEntities.id, {"data":{"lotStatus" : "CREATED"}}).then(data => {
          console.log("updated Staging entity success fully");
  ...
  getPriceDataForCenter(centerId){
    this.api.getCenterPriceSheet(centerId).then(response=>{
      console.log('price table',response);
  ...
  getUserGroupBycnteranduserIds(){
    ...
    this.api.getUserGroupData(centerId,id).then(res=>{
      ...
      console.log('user linked groupID',this.userRelatedGroupId );
    })
  }
   ...
   // }
    this.SelectedcocoonPriceValues.forEach(slectedGrade =>{
      if(slectedGrade.event.params.grade ==  this.selectedRmGrade ){
        ...
        console.log('grade selection', this.selectedGradeList);
  ...
  validateUserPrice(){
    console.log("approval process validation starts ==========>");
...
validateOTP(farmerForm){
  ...
  raiseTicketForApproval(lotForm){
    ...
    console.log(ticketParms);
    
    this.api.createApprovalTicket(ticketParms).then(response=>{
      console.log('tikt creation==>',response);
      this.$gaService.event('Cocoon Approval Request', ` ReceivedWeight =  ${lotForm.receivedWeight}, ReuestedPrice = ${lotForm.receivedWeightPricePerKg}`);
