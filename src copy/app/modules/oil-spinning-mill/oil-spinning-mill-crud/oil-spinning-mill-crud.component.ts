...
@Component({
  selector: 'app-oil-spinning-mill-crud',
  templateUrl: './oil-spinning-mill-crud.component.html',
})
export class OilSpinningMillCrudComponent implements OnInit {
  ...
  constructor(
   ...
   private toaster:ToastrService,
    ...
    this.spinningCreateForm = this.form.group({
      ...
      customerTypeName: new UntypedFormControl('',[Validators.required],),
    ...
    if (this.id) {
      this.ngxLoader.stop();
      this.api.getOilSpinningMillById(this.id).then(response => {
        ...
        if (response) {
          ...
          this.spinningCreateForm.get('customerTypeName').patchValue(this.selectedCustomerType)
          this.spinningCreateForm.get('gstnLegalName')?.patchValue(response['gstNumberProfileDetails'][0]['gstnLegalName'])
          ...
          this.spinningCreateForm.get('lastVerified')?.patchValue(this.utils.getDisplayTime(response['gstNumberProfileDetails'][0]['lastVerifiedAt']))
          ...
          this.snackBar.open('Failed to find Oil Mill', 'Ok', {
            duration: 3000
          });
        }
      });
  ...
  goBack() {
    this.router.navigate(['/resha-farms/oil-spinning-mills']);
  }
  ...
  saveSpinningDetails(spinningCreateForm) {
    ...
    const params = {
      ...
      customerType: spinningCreateForm.customerTypeName ? '/kyccustomer/' + spinningCreateForm.customerTypeName : null,
    ...
    if (this.id) {
      ...
      if (!this.gstkycVerified) {
        ...
        this.api.verifySpinningMillGstKyc(obj).then((res: any) => {
          ...
          this.api.updateOilSpinningMill(params, this.id,).then((res: any) => {
            if (res) {
              this.router.navigate(['/resha-farms/oil-spinning-mills']);
              this.toaster.success('Updated Oil  Mill successfully', 'Ok', {
                timeOut: 3000,
              })
            } else {
              this.toaster.success('Failed to Updated Oil  Mill', 'Ok', {
                timeOut: 3000,
              })
            }
          });
        ...
        this.api.updateOilSpinningMill(params, this.id,).then((res: any) => {
          if (res) {
            ...
            this.toaster.success('Failed to create Oil  Mill', 'Ok', {
              timeOut: 3000,
            })
          }
        });
  ...
  gstVerified(value) {
    const payload = {
      name:this.spinningCreateForm.get('name').value,
      phone: this.spinningCreateForm.get('phone').value,
      customerType: "/kyccustomer/" + this.spinningCreateForm.get('customerTypeName').value,
    ...
    this.ngxLoader.stop();
    this.api.gstVerificationforOilSpinningMill(payload).then((res: any) => {
      this.toaster.success('Oil mill created successfully ', 'Ok', {
        timeOut: 3000,
      })
      if (res) {
        this.res = res;
        this.id = this.res['id']
        this.router.navigate([`/resha-farms/oil-spinning-mills/crud`, this.res['id']]);
      }
    })
  }
  // verifyGst(value) {
