...
@Component({
  selector: 'app-pupae-supplier-list',
  templateUrl: './pupae-supplier-list.component.html',
  styleUrls: ['./pupae-supplier-list.component.scss']
})
export class PupaeSupplierListComponent implements OnInit {
  ...
  routeToEditPage(index){
    this.router.navigate([`/resha-farms/rm-pupae-suppliers/crud/`,this.res[index]?.id]);
  }
  
   routeToDetailspage(index){
    this.router.navigate([`/resha-farms/rm-pupae-suppliers/details/`,this.res[index]?.id]);
   }
   buildSerachQuery(searchText){
    ...
    if(searchText){
      ...
      if(searchText.includes('RMPUPAESUP')&&!isNaN(searchText.substring(10))){
...
getSuppliers(paginationData=false,searchText=''){
  this.ngxLoader.stop();
  this.apiSearch.getAllPupaeSuppliers(paginationData,this.buildSerachQuery(searchText)).then(res=>{
    this.rowDataList = [];
