...
@Component({
  selector: 'app-spinning-details',
  templateUrl: './spinning-details.component.html',
  styleUrls: ['./spinning-details.component.scss']
})
export class SpinningDetailsComponent implements OnInit {
  data: any;
  res: any;
  CONSTANT: any;
  code;
  ...
  spinningMillsdataById() {
    ...
    this.gstDetails = [];
    this.api.getSpinningMillById(this.id).then(res => {
      this.data = res;
  ...
  verifyGST() {
    let reqObj ={
      ...
      "id":parseInt(this.id),
      "cottonVendorType":"SPINNING_MILL"
