import { Component, OnInit,ChangeDetectorRef, EventEmitter, Input, OnChanges, Output, SimpleChanges } from '@angular/core';
...
@Component({
  selector: 'app-payment-tussar-purchases-khata',
  templateUrl: './payment-tussar-purchases-khata.component.html',
  styleUrls: ['./payment-tussar-purchases-khata.component.scss']
})
export class PaymentTussarPurchasesKhataComponent implements OnInit,OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    this.orderDetails.sellingPricePerPc = parseFloat(this.orderDetails?.sellingPricePerPc)
  }
  modalRef;
    ...
    getReelerKhata(){
      this.ngxLoader.stop();
      this.api.getReelerKhata(this.orderDetails.id).then((res:any)=>{
        this.mudraSalesPayment = res?._embedded?.reelerpayment.some(x=>x.paymentMode=="Mudra");
    ...
    if(this.reelerPaymentForm.get('paymentMode').value=='Self'){
      if (this.reelerPaymentForm.value.amount && this.reelerPaymentForm.value.referenceNumber) {
        const reqObj = {
          ...
          tussarSalesOrder: '/tussarsalesorder/' + this.paymentDetail.id,
          reeler: {
            "id": this.paymentDetail?.reelerData?.id
          },
          screenshotUrl: null,
          isOutOfBand: true,

        }
        this.api.tussarPayReelerDueAmount(reqObj).then(response => {
          this.reelerPaymentForm.reset();
          this.$gaService.event('Tusssar Sales Payment', ` totalAmount =  ${reqObj.amount}`);
        // this.getReelerKhata();
  ...
  masterInformation(){
    this.apiSearch.creditLoans(false,'creditlinesvc/creditlimit/master/spec',`(productType==BNPL  and customer.externalCustomerId==${this.customerId})`).then(res=>{
