...
@Component({
  selector: 'app-tussar-farmer-crud',
  templateUrl: './tussar-farmer-crud.component.html',
  styleUrls: ['./tussar-farmer-crud.component.scss'],
  providers: [
...
export class TussarFarmerCrudComponent implements OnInit,OnDestroy {
  ...
  private toaster:ToastrService,
   ...
   // this.getAllChawkis();
    ...
    this.farmerCreateForm = this.form.group({
      ...
      productType:new UntypedFormControl('Tussar',[Validators.required]),
      frontDoc:[],
      backDoc:[],
      csbFront:[],
      csbBack:[]
...
kycCustomerTypes:any = [];
  ...
  goBack() {
    this.router.navigate(['/resha-farms/tussar/tussar-farmer-list']);
  }
  ...
  updateFarmerDetails(farmerForm){
    const params = {
      ...
      bankDetails: farmerForm.bankDetails?farmerForm.bankDetails:null,
    ...
    this.api.updateFarmers(params,this.id).then((res:any) => {
      if(this.queryParams && this.queryParams?.redirecto && this.queryParams?.redirecto == 'tussar-lot-crud'){
        this.router.navigate(['/resha-farms/tussar/tussar-farmer-crud/' + this.id ],{queryParams:{redirecto:'tussar-lot-crud'}});
      }else{
        this.router.navigate(['/resha-farms/tussar/tussar-farmer-crud/' + this.id ]);
      }
    ...
    if (this.id) {
      ...
      this.api.farmersOnboarding(params).then((res:any) => {
        if (res) {
          // Success Message
          this.res=res;
          this.router.navigate(['/resha-farms/tussar/tussar-farmer-crud/' + this.res.id ]);
          this.snackBar.open('Created farmer successfully', 'Ok', {
            duration: 3000
          });
        } else {
  ...
  uploadKycDetails(){
    ...
    if(newPayload.farmerKYCAudits.length){
      this.api.apiSaveFarmerKyc(newPayload).then(ures => {
        if(!this.verified && this.queryParams && this.queryParams?.redirecto && this.queryParams?.redirecto == 'tussar-lot-crud'){
    ...
    }else if(!this.verified && this.queryParams && this.queryParams?.redirecto && this.queryParams?.redirecto == 'tussar-lot-crud'){
      this.toaster.error('Please Verify Bank Details', 'Bank Verification', {
        timeOut: 5000
      });
    }else{
  ...
  redirectoListing(){
    if(this.queryParams && this.queryParams?.redirecto && this.queryParams?.redirecto == 'tussar-lot-crud'){
      this.router.navigate(['/resha-farms/tussar/tussar-lot-crud'],{queryParams:{selectedFarmerId:this.id}});
    }else{
      this.router.navigate(['/resha-farms/tussar/tussar-farmer-list']);
    }
  }
