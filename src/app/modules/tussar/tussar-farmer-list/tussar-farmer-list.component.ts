...
@Component({
  selector: 'app-tussar-farmer-list',
  templateUrl: './tussar-farmer-list.component.html',
  styleUrls: ['./tussar-farmer-list.component.scss']
})
export class TussarFarmerListComponent implements OnInit {
  farmersList = [];
  searchedFarmers = [];
  filteredFarmersList = [];
  searchText;
  ...
  filter:UntypedFormGroup = new UntypedFormGroup({
    productType:new UntypedFormControl('Tussar'),
    ...
    if (docverified != null) {
      let query = `(productType=in=${this.productType} and eligibleForTransactions == ${docverified} `;
        if (this.searchText) {
         ...
         let query:String=`(`;
         (query+=`((name == *${text}*  or phone == *${text}* or center.centerName == *${text}*) and productType=in=${this.productType} or eligibleForTransactions == ${docverified}`);
      ...
      let query = `(productType=in=${this.productType}  `;
        if (this.searchText) {
         let text=this.searchText.replace(/ /gi,"*");
         
         let query:String=`(`;
         (query+=`((name == *${text}*  or phone == *${text}* or center.centerName == *${text}*) and productType=in=${this.productType}`);
         this.searchText.toString()?.toUpperCase()?.includes('RMFARM')&&!isNaN(this.searchText.substring(6))&&(query+=` or id==${this.searchText.substring(6)}`)
         !isNaN(parseInt(this.searchText))&&(query+=` or id==${this.searchText}`)
  ...
  createNew() {
    this.router.navigate(['/resha-farms/tussar/tussar-farmer-crud']);
  }
  open(content, item) {
    this.user = item;
  ...
  editFarmer(farmer) {
    if(this.productType == 'Cotton'){
      ...
      this.router.navigate(['/resha-farms/tussar/tussar-farmer-crud',farmer.id]);
     
    }
  }
  farmerDetail(farmer) {
      this.router.navigate(['/resha-farms/tussar/tussar-farmer-details', farmer.id]);
  }
