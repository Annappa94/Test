...
@Component({
  selector: 'app-dispatched-cocoon-lots',
  templateUrl: './dispatched-cocoon-lots.component.html',
  styleUrls: ['./dispatched-cocoon-lots.component.scss']
})
export class DispatchedCocoonLots implements OnInit {
  searchText = '';
  id = '';
  status = '';
  isTableLoaded = false;
  ...
  async getAllDispatchedLots(page, size, column, direction, status, dispatchedStatus) {
    ...
    this.api.getAllLogisticsList(page, size, column, direction, status, dispatchedStatus).then(res => {
  ...
  async onPageChange(page) {
    ...
    if (this.enableSearch) {
      let searchParams = `(payoutStatus=in=${this.seletedStatus} and driver.name==*${this.searchText.replace(/ /gi, "*")}* or driver.mobile==*${this.searchText.replace(/ /gi, "*")}*`;
  ...
  async onSort(column) {
    ...
    if (this.enableSearch) {
      let searchParams = `(paymentStatus=in=${this.seletedStatus} and driver.namee==*${this.searchText.replace(/ /gi, "*")}* or driver.mobile==*${this.searchText.replace(/ /gi, "*")}*`;
      !isNaN(parseInt(this.searchText)) && (searchParams += ` or id==${parseInt(this.searchText)}`)
      this.searchText?.includes('RMLOGDO')
        && (!isNaN(parseInt(this.searchText?.substring(11))))
        && (searchParams += ` or id==${parseInt(this.searchText?.substring(11))}`);
  ...
  lotDetails(item) {
    //e.preventDefault();
    this.router.navigate(['/resha-farms/cocoon-lot/details', item['itemId']]);
  }
