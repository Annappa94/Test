...
@Component({
  selector: 'app-cocoon-order-details',
  templateUrl: './cocoon-order-details.html',
  styleUrls: ['./cocoon-order-details.component.scss']
})
export class CocoonOrderDetailsComponent implements OnInit {
  ...
  getCocoonOrderById(id) {
    this.ngxLoader.stop();
    this.api.getCocoonOrderById(id).then(res => {
      console.log("jdsygcdscvj",res)
      ...
      this.lotList = res['_embedded']?.['cocoonorders']
      ...
      this.getReelerDetailsById(this.orderDetails.reelerId);
  ...
  farmerDetail(item) {
    this.router.navigate(['/resha-farms/farmers-silk/details', item.farmerId]);
  }
  ...
  goToReelerDetails() {
    this.router.navigate(['/reelers/details', this.reelerDetails['reeler'].id]);
  }
  
  getReelerKhata() {
    this.ngxLoader.stop();
    this.api.getCocoonReelerKhataforPayments(this.orderDetails.id).then(res => {
      ...
      this._cd.detectChanges();
      console.log('res',this.reelerKhata)
    }) 
  }
