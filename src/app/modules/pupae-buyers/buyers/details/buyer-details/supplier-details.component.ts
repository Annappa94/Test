...
export class BuyerDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getAllFarmerKycDocList(this.id , 'pupaesvc/pupaebuyer').then(res => {
      this.noOfKYCDocs = res['_embedded']['pupaebuyerkyc'];
  ...
  ConfirmDeleteRecord(){
    ...
    this.api.deleteKYCByPerforma('pupaeBuyer', this.id,'/pupaesvc/kyc').then(res=>{
      if(res){
  ...
  navigateToKyc(){
    if(this.farmerDetails.customerTypeName){
      this.router.navigate(['/resha-farms/kyc-form',this.id,'pupaebuyer',this.farmerDetails.customerTypeName])
    } else {
  ...
  editFarmer() {
    this.router.navigate(['/resha-farms/rm-pupae-buyers/crud', this.farmerDetails.id]);
  }
